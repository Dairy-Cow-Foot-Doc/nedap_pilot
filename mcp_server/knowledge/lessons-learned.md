...
- **The external chronicity classifier's `Repeat` vs. `Chronic` cutoff is a 1-day gap.** In practice this means almost any cow with 2+ lifetime lesion diagnoses ends up `Chronic`, regardless of whether those diagnoses were weeks or years apart (in the 2-arm data: 180 `Chronic` vs. only 2 `Repeat`, out of 434 cows). Don't be surprised if `Repeat` is nearly empty again in the 3-arm data — that's the classifier's definition, not a bug.
- **A cow with zero lifetime lameness diagnosis is folded into `New`, not given a separate 4th category.** This was an explicit modeling choice (not wanting an artificial 4th bucket alongside the classifier's real New/Repeat/Chronic), made after checking that even 5 years of history still leaves ~37% of cows with no prior diagnosis at all.
